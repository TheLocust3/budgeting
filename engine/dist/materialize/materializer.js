...
const buildNumberPredicate = (field) => (pred) => {
    if (field == "capturedAt") {
        return (transaction) => O.match(() => false, (capturedAt) => pred(capturedAt))(transaction.capturedAt);
    }
    else {
        return (transaction) => pred(transaction[field]);
    }
...
const buildAttach = (rule) => {
    ...
    return (transaction) => {
        if (where(transaction)) {
            try {
                return Object.assign(Object.assign({}, transaction), { custom: Object.assign(Object.assign({}, transaction.custom), { [rule.field]: transaction.custom[rule.field].concat(rule.value) }) });
            }
            catch (_) { // if [rule.field] key doesn't exist
                return Object.assign(Object.assign({}, transaction), { custom: Object.assign(Object.assign({}, transaction.custom), { [rule.field]: [rule.value] }) });
            }
        }
        else {
            return transaction;
    ...
    return (transaction) => {
        if (where(transaction)) {
            const tagged = A.map((split) => {
                const splitTransaction = Object.assign(Object.assign({}, transaction), { amount: transaction.amount * split.percent });
            ...
            const [remaining, tagged] = A.reduce([transaction.amount, []], ([remaining, tagged], split) => {
                if (remaining > 0) {
                    if (remaining >= split.value) {
                        const splitTransaction = Object.assign(Object.assign({}, transaction), { amount: split.value });
                        return [remaining - split.value, tagged.concat({ _type: "Tagged", tag: split.account, element: splitTransaction })];
                    }
                    else {
                        const splitTransaction = Object.assign(Object.assign({}, transaction), { amount: remaining });
...
const buildSplitFlow = (split) => {
    ...
    return (transaction) => {
        const [_, out] = (0, pipeable_1.pipe)(splitFlows, A.map(([split, flow]) => [split, flow(transaction)]), A.reduce([O.none, { _type: "Untagged", element: transaction }], ([last, out], [split, maybeTagged]) => O.match(() => [last, out], (tagged) => {
            switch (out._type) {
                case "Conflict":
                    return [O.some(split), Object.assign(Object.assign({}, out), { rules: out.rules.concat(split) })];
                case "TaggedSet":
