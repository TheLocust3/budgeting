...
export const byId = (pool: Pool) => (log: Logger) => (id: string) : TE.TaskEither<Exception.t, O.Option<Account.Internal.t>> => {
  return pipe(
      TE.tryCatch(
...
export const childrenOf = (pool: Pool) => (log: Logger) => (parent: string) : TE.TaskEither<Exception.t, string[]> => {
  return pipe(
      TE.tryCatch(
