...
export const byAccountId = (pool: Pool) => (userId: string) => (accountId: string) : TE.TaskEither<Exception.t, Rule.Internal.t[]> => {
  return pipe(
      TE.tryCatch(
...
export const byId = (pool: Pool) => (id: string) : TE.TaskEither<Exception.t, O.Option<Rule.Internal.t>> => {
  return pipe(
      TE.tryCatch(
