...
export type Passthrough = {
  putObject: <T>(formatter: Format.Formatter<T>) => (writeFunc: (savedObject: O.Option<T>) => T) => (path: string) => TE.TaskEither<Exception.t, T>;
  getObject: <T>(formatter: Format.Formatter<T>) => (path: string) => TE.TaskEither<Exception.t, T>;
...
export class FilePassthrough implements Passthrough {
  ...
  public getObject = <T>(formatter: Format.Formatter<T>) => (path: string): TE.TaskEither<Exception.t, T> => {
    return TE.throwError(Exception.throwNotFound);
  }
